...
//function Main () {
 ...
 function addContent(arg) {
  ...
  function makeAPost (idStr, contentStr) {
   ...
   div.className = 'post'
   div.id = idStr.replace(/\'/g,'-')
   div.id = div.id.replace(/\s/g,'_')
 ...
 function saveContent() {
   ...
   posts.forEach( function (post) {
     ...
     row_items.forEach( function (item) {
       ...
       var item_tuple = {
         text:item.innerHTML.replace(/\'/g,"&apos;"),  //TRY THIS AGAIN: the idea is there but not universally applied to all entities and not re-interpreted in the other direction
         // look at the router - what's it doing with the raw data? look at the CRUD controller{s) - are they for real?
        // text:item.innerText,  //this crashes the app silently when a string delimiter is introduced into the form field - why doesn't it just terminate the string early?
         toggle:this_toggle
       }
 ...
 function buildContent(json) {
   ...
   data.forEach( function (post) {
     var this_id = post.textspan.replace(/\'/g,"-")
     this_id = this_id.replace(/\s/g,'_')
